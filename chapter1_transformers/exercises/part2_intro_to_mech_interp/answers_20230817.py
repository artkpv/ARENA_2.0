...
actual_tokens = gpt2_small.to_tokens(model_description_text).squeeze()
correct = prediction == actual_tokens[1:]
pp(f'correct ration: {(correct).sum()} / {prediction.size(0)}')
pp(f'Correct: {gpt2_small.to_str_tokens(prediction[correct])}' )
# %%
...
def run():
    ...
    q_n_k = einops.einsum(gpt2_cache['q', 0], gpt2_cache['k', 0], 'sq hi dh, sk hi dh -> hi sq sk')
    q_n_k.masked_fill_(t.triu(t.ones_like(q_n_k), diagonal=1).bool(), -1e9)
    q_n_k /= cfg.d_head ** .5
    q_n_k = t.softmax(q_n_k, dim=-1)
    layer0_pattern_from_q_and_k = q_n_k
...
def visualise_attention_patterns(layer=0):
...
visualise_attention_patterns(0)
visualise_attention_patterns(1)
# %%
...
def current_attn_detector(cache: ActivationCache) -> List[str]:
    ...
    for layer in range(cfg.n_layers):
        ...
        for hi in range(pattern.size(0)):
            if pattern[hi].diagonal().mean() > 0.4:
                res += [f"{layer}.{hi}"]
    return res
...
def prev_attn_detector(cache: ActivationCache) -> List[str]:
    ...
    for layer in range(cfg.n_layers):
        pattern = cache["pattern", layer]
        for hi in range(pattern.size(0)):
            if pattern[hi].diagonal(offset=-1).mean() > 0.4:
                res += [f"{layer}.{hi}"]
    return res
...
def induction_attn_detector(cache: ActivationCache) -> List[str]:
    ...
    for layer in range(cfg.n_layers):
        pattern = cache["pattern", layer]
        seq_n = (pattern.size(-1) - 1) // 2
        for hi in range(pattern.size(0)):
            if pattern[hi].diagonal(offset=-(seq_n-1)).mean() > 0.4:
                res += [f"{layer}.{hi}"]
    return res
...
def induction_score_hook(
    ...
    '''
    batch, hi, dp, sp = pattern.shape
    seq_n = (sp - 1) // 2
    res = pattern.diagonal(dim1=-2, dim2=-1, offset=-(seq_n-1))
    induction_score_store[hook.layer(), :] = einops.reduce(res, 'b hi pos -> hi', 'mean')
    return pattern
...
def logit_attribution(
    ...
    seq, nheads, dmodel = l1_results.shape
    res = t.zeros((seq-1, nheads*2 + 1))
    res[:, 0] = einops.einsum(embed[:-1], W_U_correct_tokens, 's dm, dm s -> s')
    res[:, 1:1+nheads] = einops.einsum(l1_results[:-1], W_U_correct_tokens, 's h dm, dm s -> s h')
    res[:, 1+nheads:] = einops.einsum(l2_results[:-1], W_U_correct_tokens, 's h dm, dm s -> s h')
    return res
...
def head_ablation_hook(
    ...
    batch, dp, sp = v[:, :, head_index_to_ablate, :].shape
    seq_n = (sp - 1) // 2
    is_prev_head = v[:, :, head_index_to_ablate, :].diagonal(offset=-(seq_n-1)).mean() > 0.4

    induct_diag = v[:, :, head_index_to_ablate, :].diagonal(dim1=-2, dim2=-1, offset=-(seq_n-1))
    is_ind_head =  einops.reduce(induct_diag, 'b pos -> ', 'mean') > 0.4

    #if is_prev_head or is_ind_head:
    #    v[:, :, head_index_to_ablate, :] = 0.0
        #v[:, :, head_index_to_ablate, :] = v[:, :, head_index_to_ablate, :].mean().float()
    v[:, :, head_index_to_ablate, :] = 0.0
    return v
