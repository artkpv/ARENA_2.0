...
if MAIN:
    ...
    full_OV_circuit = (
        model.W_E @ FactoredMatrix(model.W_V[l, h], model.W_O[l, h]) @ model.W_U
    )
...
def top_1_acc(full_OV_circuit: FactoredMatrix, batch_size: int = 100) -> float:
    ...
    for i in tqdm(range(0, nrows + batch_size, batch_size)):
        ...
        if rng:
            ...
            nrows_max_on_diagonal += (
                (submatrix.argmax(-1) == diag_indices).float().sum().item()
            )
...
if MAIN:
    ...
    scores = mask_scores(pos_by_pos_pattern / model.cfg.d_head**0.5)
...
def decompose_q(decomposed_qk_input: t.Tensor, ind_head_index: int) -> t.Tensor:
    ...
    return einops.einsum(
        decomposed_qk_input, model.W_Q[1, ind_head_index], "i p dh, dh dm -> i p dm"
    )
...
def decompose_k(decomposed_qk_input: t.Tensor, ind_head_index: int) -> t.Tensor:
    ...
    """
    return einops.einsum(
        decomposed_qk_input, model.W_K[1, ind_head_index], "i p dh, dh dm -> i p dm"
    )
...
def decompose_attn_scores(decomposed_q: t.Tensor, decomposed_k: t.Tensor) -> t.Tensor:
    ...
    """
    return einops.einsum(decomposed_q, decomposed_k, "i qp dm, j kp dm -> i j qp kp")
...
def find_K_comp_full_circuit(
    ...
    """
    query = FactoredMatrix(model.W_E, model.W_Q[1, ind_head_index])
    key = (
        model.W_E
        @ FactoredMatrix(
            model.W_V[0, prev_token_head_index], model.W_O[0, prev_token_head_index]
        )
